...
- **历史记录管理**：可单独删除历史对话，也可在设置中经过二次确认后清空全部记录并重置当前对话
...
历史记录使用 schema v3，并为旧数据自动补充默认人设和联网模式，同时忽略旧版本遗留的学习模式字段。最近选择的人设与联网模式另存于 `chat-o-matic-preferences`，刷新页面和新建对话时继续沿用；主题偏好继续独立保存。
