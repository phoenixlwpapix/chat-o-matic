...
- **友好错误反馈**：额度耗尽、访问受限或上游故障时，在对话区显示可关闭的中文提示，不再暴露原始 403/错误页
- **Markdown 渲染**：完整支持 Markdown 格式，包括表格、代码块和任务列表
- **Google Gemini**：由 Gemini 3.5 Flash-Lite 模型驱动
- **AI 人设切换**：5 种个性鲜明的聊天角色（学习伙伴、科学怪博士、冒险故事家、哲学喵、吐槽达人），在统一设置面板中随时切换
- **人设专属开场卡片**：20 张快捷卡片按学习辅导、科学探究、互动叙事、哲思交流和轻松吐槽重新分工，并强化实验安全与连续互动
- **专注式侧边栏**：左侧只保留新建对话、当前人设、收藏、历史记录和底部设置入口；有收藏时自动置顶单列显示，取消收藏后回到历史记录
- **统一偏好设置**：人设、联网模式和主题配色集中管理并立即保存；切换人设时会自动保存当前对话
- **本地头像裁剪**：可从设备选择 JPG、PNG 或 WebP 图片，拖动缩放完成圆形头像裁剪；结果仅保存在当前浏览器
- **历史记录管理**：最多保留 20 条对话，可单独删除，也可在设置中经过二次确认后清空全部记录并重置当前对话
- **对话收藏夹**：最多收藏 10 条历史对话；记录超出上限时优先清理最旧的未收藏对话
- **联网控制**：在设置中选择自动联网、强制联网或关闭联网；请求期间以动态图标实时区分“思考中”和“联网搜索中”，并明确标识每次回复是否使用了联网参考
- **统一回复操作栏**：用紧凑小标签一键简化、举例、出题、联网检查事实、重新生成或复制回复
- **完整 PWA 品牌图标**：统一使用闪电机器人 Logo，并提供 favicon、Apple Touch Icon 与 maskable 安装图标
...
- **响应式设计**：完美适配桌面端和移动端
- **移动端沉浸式聊天**：全屏动态视口适配 iPhone 键盘与安全区，宽版 AI 消息和一体式输入栏减少遮挡并提升正文可读性
...
历史记录使用 schema v4，并为旧数据自动补充默认人设、联网模式和收藏状态，同时忽略旧版本遗留的学习模式字段。历史记录最多保留 20 条，收藏夹最多保留 10 条；达到历史上限后新增对话时，会优先清理最旧的未收藏记录。最近选择的人设与联网模式另存于 `chat-o-matic-preferences`，刷新页面和新建对话时继续沿用；主题偏好继续独立保存。
